...
public class SiemensFireviewChannelAdapter extends NettyTcpServer{
	...
	protected void initChannelPipeline(ChannelPipeline pipeline) throws Exception {
		...
		pipeline.addLast("logging", new LoggingHandler(getClass()));
		...
		pipeline.addLast("read_message_decode",new ByteToMessageDecoder(){
			protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
				...
				if(in.readableBytes() == 4){
					...
					*/
					out.add(new SiemensFireviewResponse(data, op, OP_Codes.get(op)));
				}
				
				ctx.writeAndFlush(ACK.copy());
			}
		});
		pipeline.addLast("channelHandler", new NettyMessageHandler(sendChannel, null, 1000));
	}
